...
*       LogicalMinus(all=[false])
    ...
    =>
    LogicalProject(car_id=[$0])
      LogicalFilter(condition=[AND(OR(<>($3, 0), >($1, 0)), OR(<=($1, 0), =($3, 0)))])
        LogicalTableScan(table=[[shenzhen0, Car]])
 */

@Value.Enclosing
public class MinusTwoFilterRule extends RelRule<MinusTwoFilterRule.Config> implements SubstitutionRule {
    


    protected MinusTwoFilterRule(Config config) {
        super(config);
    }
    
    @Deprecated
    public MinusTwoFilterRule(RelBuilderFactory relBuilderFactory) {
    ...
    @Override public void onMatch(RelOptRuleCall call) {
        ...
        // 构建替代子树：Project(Select cond1 and not cond2 (Scan))
        RelNode replacement = builder.push(leftScan)
                .filter(builder.and(leftFilter.getCondition(), builder.not(rightFilter.getCondition())))
                .project(builder.field(leftProj.getRowType().getFieldNames().get(0)))
                .build();


        // 保持与原顶层 Minus 相同的 convention
        replacement = convert(call.getPlanner(), replacement, topMinus.getConvention());

        // 替换为 Project(Select cond1 and not cond2 (Scan))
     ...
     * 
     */
    public static boolean isSameSourceAndSameColumn(LogicalMinus minus) {
    ...
    public interface Config extends RelRule.Config {
        Config DEFAULT = ImmutableMinusTwoFilterRule.Config.builder().build()
            .withOperandSupplier(b0 -> b0.operand(LogicalMinus.class)
                                        .predicate(MinusTwoFilterRule::isSameSourceAndSameColumn)
                                        .inputs(
        ...
        @Override default MinusTwoFilterRule toRule() {
            return new MinusTwoFilterRule(this);
        }
    }
